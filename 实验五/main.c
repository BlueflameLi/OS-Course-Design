...
// 清理父目录空余表项
void clearfa(char *dirname, int dirno, int diroff);
...
const char USERNAME[] = "Blueflame"; //用户名
int BLOCKSIZE;                       //磁盘块大小
int BLOCKNUM;                        //磁盘块总数
...
int main()
{
    int fd;
    int pos;
    char cmdline[DIRLEN] = "", *command;
    startsys();
    printf("%s%s %s%s%s$ ", GREEN, USERNAME, BLUE, openfilelist[curdirid].dir, RESET);
    while (~scanf("%[^\n]%*c", cmdline))
    {
        if (strcmp(cmdline, ""))
        {
            ...
            else if (!strcmp(command, "open"))
            {
                ...
                if (0 <= fd && fd < MAXOPENFILE)
                {
                    ...
                    {
                        my_close(fd);
                        printf("%s 请使用 cd 命令来打开目录\n", command);
                    }
                    else
                        printf("%s 文件已打开, fd 为 %d\n", openfilelist[fd].dir, fd);
                }
            }
            ...
            else if (!strcmp(command, "sf"))
            {
                ...
                my_format();
            else if (!strcmp(command, "showfat"))
                show_fat();
            else if (!strcmp(command, "changesize"))
                change_block_size();
            else
                printf("命令 %s 不存在\n", command);
        }
...
void fcb_init(fcb *new_fcb, const char *filename, unsigned short first, unsigned char attribute)
{
    ...
    struct tm *lc_time = localtime(&raw_time);
    new_fcb->time = lc_time->tm_hour << 11 | lc_time->tm_min << 5 | lc_time->tm_sec >> 1;
    new_fcb->date = (lc_time->tm_year - 80) << 9 | lc_time->tm_mon << 5 | lc_time->tm_mday;
    if (attribute == 1)
    {
...
void my_ls()
{
    ...
    for (int i = 0; i < read_size; i += sizeof(fcb))
    {
        ...
        if (dirfcb.attribute)
        {
            printf("%s\t%ldB\t<File>\t%d/%d/%d\t%02d:%02d:%02d\n",
                   dirfcb.filename, dirfcb.length,
                   (dirfcb.date >> 9) + 1980,
                   ((dirfcb.date >> 5) & 0x000f) + 1,
                   (dirfcb.date) & 0x001f,
                   (dirfcb.time >> 11),
                   (dirfcb.time >> 5) & 0x003f,
                   (dirfcb.time) & 0x001f * 2);
        }
        else
        {
            printf("%s%s\t    \t<DIR>\t%d/%d/%d\t%02d:%02d:%02d%s\n",
                   GREEN,
                   dirfcb.filename,
                   (dirfcb.date >> 9) + 1980,
                   ((dirfcb.date >> 5) & 0x000f) + 1,
...
int my_write(int fd, int pos)
{
    ...
    while (~scanf("%[^\n]%*c", content))
    {
        ...
        content[len++] = '\n';
        tmp = do_write(fd, (unsigned char *)content, len, op[0]);
        if (tmp == -1)
        {
            printf("ERROR:\n");
...
void my_rmdir(char *dirname)
{
    ...
    openfilelist[fd].open_fcb.free = 2;
    fatFree(openfilelist[fd].open_fcb.first);
    openfilelist[fd].fcbstate = 1;
    my_close(fd);
    clearfa(dirname, openfilelist[fd].dirno, openfilelist[fd].diroff);
}
...
void my_rm(char *filename)
{
    ...
    openfilelist[fd].fcbstate = 1;
    my_close(fd);
    clearfa(filename, openfilelist[fd].dirno, openfilelist[fd].diroff);
}

void my_format()
{
    printf("请输入磁盘块大小：\n");
    while (~scanf("%d", &BLOCKSIZE))
    {
        if (BLOCKSIZE > 1024 || BLOCKSIZE < 256)
            printf("磁盘块大小必须在256~1024范围内\n");
        else
        {
...
int my_touch(char *filename, int attribute, int *rpafd)
{
    ...
    if (attribute == 0)
    {
        int len = strlen(openfilelist[fd].dir);
        if (openfilelist[fd].dir[len - 1] != '/')
        {
...
void my_cd(char *dirname)
{
    ...
    my_close(curdirid);
    openfilelist[curdirid].topenfile = 0;
    curdirid = fd;
}
...
void clearfa(char *dirname, int dirno, int diroff)
{
    // 先打开file的上级目录，如果上级目录不存在就报错
    char split_dir[2][DIRLEN];
    splitLastDir(dirname, split_dir);
    int pafd = my_open(split_dir[0]);
    if (pafd < 0 || pafd >= MAXOPENFILE)
    {
        printf("ERROR\n");
    ...
    if (flag)
    {
        ...
        if (diroff)
            fat1[dirno].id = END;
        openfilelist[pafd].open_fcb.length = flag;
        openfilelist[pafd].fcbstate = 1;
    }
    my_close(pafd);
}