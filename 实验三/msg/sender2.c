...
int main()
{
    ...
    // sem_wait(mutex);
    if (msgrcv(msqid, (void *)&msgp, MSG_MAX, SENDER2, 0) == -1)
    {
        printf("sender2_receive消息接收失败\n");
        exit(-1);
    }
    printf("收到了来自进程%d的消息：%s\n", msgp.sendid, msgp.mtext);
    sem_post(sender2_receive);
    // sem_post(mutex);
    return 0;
}